...
const Notifications = () => {
  ...
  // 获取用户通知
  useEffect(() => {
    if (user && profile && isLoggedIn && role === 'student') {
      loadNotifications();
    }
  ...
  const loadNotifications = async () => {
    ...
    try {
      ...
      if (result.success) {
        // 只显示学生可见的通知
        const studentNotifications = result.data.filter(n => {
          const config = NOTIFICATION_CONFIG[n.type];
          return config?.studentVisible;
        });
  ...
  const handleMarkAllAsRead = async () => {
    try {
      ...
      if (result.success) {
        ...
        refreshUnreadCount();
        toast.success(language === "zh" ? '已全部标记为已读' : 'Marked all as read');
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-blue-100 to-slate-50">
      ...
      <Navbar title={language === "zh" ? "我的通知" : "My Notifications"} />
        ...
        <div className="max-w-2xl mx-auto">
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">{language === "zh" ? "我的通知" : "My Notifications"}</h1>
          ...
          >
            <p className="text-sm text-gray-400">
              {language === "zh" ? "通知将显示社团申请、退出申请和社团动态等信息" : "Notifications show club applications, leave requests, and club updates"}
            </p>
          </motion.div>
        </div>
      </main>
